...
from utils.waste_calculator import (
    calculate_waste_statistics,
    get_waste_percentage_color,
)
...
try:
    from utils.ml_predictor import (
        ...
        train_ml_model,
        get_training_status,
        add_project_to_training,
        collect_training_data_from_projects,
        is_in_training,
        remove_project_from_training,
        remove_elevation_from_training,
    )
...
def create_bay_diagram_base64(
    ...
    if custom_bay_widths and len(custom_bay_widths) == bays_wide:
        bay_widths = [
            max(0.1, float(w)) for w in custom_bay_widths
    ...
    if custom_bay_heights and len(custom_bay_heights) == bays_tall:
        bay_heights = [
            max(0.1, float(h)) for h in custom_bay_heights
        ]  # Ensure minimum positive value
    else:
    ...
    draw.text(
        (diagram_width // 2, 15), title, fill="#0073E6", anchor="mm", font=font_large
    )

    # Add C/L Dimensions note
    cl_note = "C/L Dimensions"
    draw.text(
        (diagram_width // 2, 30), cl_note, fill="#B3B3B3", anchor="mm", font=font_small
    )
    ...
    for row in range(bays_tall):
        ...
        for col in range(bays_wide):
            ...
            draw.rectangle([x0, y0, x1, y1], outline="#0073E6", width=2)
            ...
            if bay_w > 4 and bay_h > 4:
                ...
                if fill_x1 > fill_x0 and fill_y1 > fill_y0:
                    draw.rectangle([fill_x0, fill_y0, fill_x1, fill_y1], fill="#2A2A2A")
    ...
    if (
        ...
        for left_in, right_in, label, door_h_in in _door_x_spans(doors, opening_width):
            ...
            door_labels.append(
                f'{label}: Length {left_in:.1f}"-{right_in:.1f}" occupied'
            )
...
def main(page: ft.Page):
    ...
    def prepare_temp_files_for_excel(project_name, elevations_override=None):
        ...
        elevations = (
            elevations_override
            if elevations_override is not None
            else db.get_elevations(project_name)
        )
    ...
    def create_input_field(
        label, key, expand=True, value="", numeric=False, on_change=None
    ):
        field = ft.TextField(
            label=label,
            value=value,
    ...
    def show_snack(msg, color="white"):
        page.snack_bar = ft.SnackBar(
            content=ft.Text(msg, color=COLOR_TEXT), bgcolor=COLOR_SURFACE
        )
    ...
    def build_projects_view():
        ...
        def get_project_metadata(project_name):
            ...
            metadata = {"systems": set(), "finishes": set(), "elevation_count": 0}
            try:
            ...
            def do_delete(e):
                if name in state["projects"]:
                    ...
                    if ML_AVAILABLE and removed_count > 0:
                        show_snack(
                            f"Project '{name}' deleted ({removed_count} training samples removed)",
                            "red",
                        )
                    else:
        ...
        system_filter = ft.Dropdown(
            ...
            + [ft.dropdown.Option(s) for s in all_systems],
            value="All Systems",
            width=240,
            bgcolor="#0D0D0D",
            border_color="#333333",
        ...
        finish_filter = ft.Dropdown(
            ...
            options=[ft.dropdown.Option("All Finishes")]
            + [ft.dropdown.Option(f) for f in all_finishes],
            value="All Finishes",
            width=180,
            bgcolor="#0D0D0D",
            border_color="#333333",
        ...
        result_count_text = ft.Container(
            content=ft.Row(
                [
                    ft.Container(
                        content=ft.Icon(
                            ft.Icons.INVENTORY_2_OUTLINED, size=14, color=COLOR_ACCENT
                        ),
                        padding=4,
                    ),
        ...
        def apply_filters(e=None):
            ...
            for p in state["projects"]:
                ...
                # Filter by system
                meta = project_metadata_cache.get(
                    p, {"systems": set(), "finishes": set()}
                )
            ...
            for p in filtered_projects:
                meta = project_metadata_cache.get(
                    p, {"elevation_count": 0, "systems": set(), "finishes": set()}
                )
                                ...
                                [
                                    ...
                                    ft.Row(
                                        [
                                            ...
                                            ft.IconButton(
                                                ...
                                                on_click=lambda e, name=project_name: (
                                                    delete_project_click(e, name)
                                                ),
                                            ),
                                        ],
                                    ...
                                    ft.Row(
                                        [
                                            ...
                                            ft.Text(
                                                f"{elevation_count} elevation{'s' if elevation_count != 1 else ''}",
                                                size=11,
                                                color=COLOR_TEXT_DIM,
                                                weight=ft.FontWeight.W_400,
                                            ),
            ...
            # Update result count
            result_count_text_widget.value = (
                f"Showing {len(filtered_projects)} of {len(state['projects'])} projects"
            )
        ...
        empty_state = ft.Container(
            content=ft.Column(
                [
                    ft.Icon(
                        ft.Icons.FOLDER_OFF_OUTLINED, size=80, color=COLOR_TEXT_DIM
                    ),
                                    ...
                                    [
                                        ...
                                        ft.Column(
                                            [
                                                ...
                                                ft.Text(
                                                    "Professional Storefront & Curtain Wall Estimator",
                                                    size=16,
                                                    color=COLOR_TEXT_DIM,
                                                    weight=ft.FontWeight.W_400,
                                                ),
                                    ...
                                    [
                                        ...
                                        ft.Container(
                                            content=ft.ElevatedButton(
                                                ...
                                                on_click=lambda e: (
                                                    page.go("/ml_analytics")
                                                    if ML_AVAILABLE
                                                    else show_snack(
                                                        "ML not available. Install scikit-learn.",
                                                        "red",
                                                    )
                                                ),
                                                height=42,
    ...
    def build_ml_analytics_view():
                    ...
                    ft.Container(
                        content=ft.Column(
                            [
                                ...
                                ft.Text(
                                    "Please install scikit-learn: pip install scikit-learn numpy",
                                    color=COLOR_TEXT_DIM,
                                ),
                                ft.ElevatedButton(
                                    "Back to Projects", on_click=lambda e: page.go("/")
                                ),
                            ],
                            alignment=ft.MainAxisAlignment.CENTER,
                            horizontal_alignment=ft.CrossAxisAlignment.CENTER,
        ...
        ml_status_text = ft.Text(
            "Ready to load projects", size=14, color=COLOR_TEXT_DIM
        )
        ...
        projects_empty_state = ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Text(
                        "Click 'Load Projects' to see predictions",
                        size=12,
                        color=COLOR_TEXT_DIM,
                    ),
                ],
        ...
        insights_empty_state = ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Text(
                        "Train the model to see pattern insights",
                        size=12,
                        color=COLOR_TEXT_DIM,
                    ),
                ],
        ...
        def refresh_ml_status():
            ...
            if sample_count < 3:
                ml_minimum_warning.content.controls[
                    1
                ].value = f"Need minimum of 3 samples to train. Currently have {sample_count} sample(s)."
        ...
        def add_single_to_training(
            elev_data, project_name, elev_name, button_ref, container_ref=None
        ):
                ...
                try:
                    ...
                    for elev_name, elev_data in elevations.items():
                        ...
                        elevation_checkbox = ft.Checkbox(
                            ...
                            on_change=lambda e, key=elev_key, data=elev_data, proj=project_name, en=elev_name: (
                                on_checkbox_change(e, key, data, proj, en)
                            ),
                        ...
                        add_button.on_click = (
                            lambda e, ed=elev_data, pn=project_name, en=elev_name, btn=add_button, cont=project_container: (
                                add_single_to_training(ed, pn, en, btn, cont)
                            )
                        )

                        # Track if in training in selected_elevations
                        if in_training:
                            selected_elevations[elev_key] = {
                                "data": elev_data,
                                "project": project_name,
                                "elev_name": elev_name,
                            }
        ...
        def select_all_projects(e):
            ...
            for container in ml_projects_list.controls:
                if (
                    isinstance(container.content, ft.Row)
                    and len(container.content.controls) >= 2
                ):
        ...
        def add_selected_to_training(e):
            ...
            for elev_key, info in selected_elevations.items():
                result = add_project_to_training(
                    info["data"], info["project"], info.get("elev_name", "")
                )
                if result:
                    added += 1
                else:
            ...
            if added > 0:
                ...
                show_snack(
                    f"All selected elevations already in training data", COLOR_TEXT_DIM
                )
            else:
        ...
        def remove_selected_from_training(e):
            ...
            not_in_training = 0
            for elev_key, info in selected_elevations.items():
                if remove_elevation_from_training(
                    info["data"], info["project"], info.get("elev_name", "")
                ):
                    removed += 1
                else:
            ...
            if removed > 0:
                show_snack(
                    f"Removed {removed} elevation(s) from training data!", "green"
                )
                refresh_ml_status()
                refresh_all_projects()
                                    ...
                                    [
                                        ...
                                        ft.Column(
                                            [
                                                ...
                                                ft.Text(
                                                    f"{patterns.get('avg_sqft', 0):.0f} sqft",
                                                    size=16,
                                                    weight="bold",
                                                    color=COLOR_TEXT,
                                                ),
                                        ...
                                        ft.Column(
                                            [
                                                ...
                                                ft.Text(
                                                    f'{patterns.get("avg_width", 0):.0f}" x {patterns.get("avg_height", 0):.0f}"',
                                                    size=14,
                                                    weight="bold",
                                                    color=COLOR_TEXT,
                                                ),
                        ...
                        content=ft.Column(
                            [
                                ...
                                ft.Row(
                                    [
                                        ft.Text(
                                            f"Min: ${patterns.get('min_cost', 0):,.2f}",
                                            size=12,
                                            color=COLOR_TEXT,
                                        ),
                                        ft.Text(" - ", color=COLOR_TEXT_DIM),
                                        ft.Text(
                                            f"Max: ${patterns.get('max_cost', 0):,.2f}",
                                            size=12,
                                            color=COLOR_TEXT,
                                        ),
                                    ],
                    ...
                    config_items.append(
                        ft.Row(
                            [
                                ...
                                ft.Text(
                                    f"{c['count']} projects ({c['percentage']:.0f}%)",
                                    size=11,
                                    color=COLOR_TEXT_DIM,
                                ),
                            ]
    ...
    def build_workspace_view():
        ...
        def set_workspace_mode(mode):
            ...
            if inputs.get("door_x_pos_container"):
                inputs[
                    "door_x_pos_container"
                ].visible = not door_only  # Door only: no X, doors adjacent
            ...
            if inputs.get("diagram_switcher_container"):
                if door_only:
                    inputs["diagram_switcher_container"].content = inputs[
                        "door_only_diagram_container"
                    ]
                    refresh_door_only_diagram()
                else:
                    inputs["diagram_switcher_container"].content = inputs[
                        "bay_diagram_container"
                    ]
                    update_bay_visibility(None)
            elif not door_only:
                update_bay_visibility(None)
            if inputs.get("toggle_elevation_btn"):
                inputs["toggle_elevation_btn"].style = ft.ButtonStyle(
                    bgcolor=COLOR_ACCENT if not door_only else COLOR_SURFACE,
                    color="white" if not door_only else COLOR_TEXT,
                )
            if inputs.get("toggle_door_only_btn"):
                inputs["toggle_door_only_btn"].style = ft.ButtonStyle(
                    bgcolor=COLOR_ACCENT if door_only else COLOR_SURFACE,
                    color="white" if door_only else COLOR_TEXT,
                )
            if page.views:
                page.update()
        ...
        def update_bay_visibility(e):
            ...
            if inputs.get("custom_w_container"):
                inputs["custom_w_container"].visible = show_bays
            if inputs.get("custom_h_container"):
                inputs["custom_h_container"].visible = show_bays
        ...
        def update_dynamic_bay_inputs(e):
            ...
            if bw > 0:
                inputs["custom_w_col"].controls.append(
                    ft.Text(
                        "Custom Bay Widths (leave blank to auto-fill)",
                        size=12,
                        color=COLOR_TEXT_DIM,
                    )
                )
                for i in range(bw):
                    ...
                    field = create_input_field(
                        ...
                        expand=True,
                        value=val,
                        on_change=lambda e: auto_refresh_bay_diagram(),
                    )
                ...
                def auto_fill_w(e):
                    ...
                    if filled_sum > total_w:
                        show_snack(
                            f"Error: Filled widths ({filled_sum:.2f}) exceed total ({total_w:.2f})",
                            "red",
                        )
                        return
                    ...
                    if blank_count > 0:
                        ...
                        if abs(filled_sum - total_w) > 0.01:
                            show_snack(
                                f"Warning: Sum ({filled_sum:.2f}) does not match Total ({total_w:.2f})",
                                "orange",
                            )
                        else:
            ...
            if bh > 0:
                inputs["custom_h_col"].controls.append(
                    ft.Text(
                        "Custom Bay Heights (leave blank to auto-fill)",
                        size=12,
                        color=COLOR_TEXT_DIM,
                    )
                )
                ...
                def auto_fill_h(e):
                    ...
                    if filled_sum > total_h:
                        show_snack(
                            f"Error: Filled heights ({filled_sum:.2f}) exceed total ({total_h:.2f})",
                            "red",
                        )
                        return
                    ...
                    if blank_count > 0:
                        ...
                        if abs(filled_sum - total_h) > 0.01:
                            show_snack(
                                f"Warning: Sum ({filled_sum:.2f}) does not match Total ({total_h:.2f})",
                                "orange",
                            )
                        else:
                            show_snack("Auto-fill complete.", "green")
        ...
        def auto_refresh_bay_diagram():
            ...
            try:
                ...
                try:
                    bw = (
                        int(inputs["bays_wide"].value)
                        if inputs["bays_wide"].value
                        else 0
                    )
                    bh = (
                        int(inputs["bays_tall"].value)
                        if inputs["bays_tall"].value
                        else 0
                    )
                except:
                    bw, bh = 0, 0
                ...
                if diagram_b64 and inputs.get("bay_diagram_image"):
                    ...
                    print(
                        f"[Bay Diagram] Updated: {bw}W x {bh}H, opening={opening_w}x{opening_h}, custom_w={final_custom_w is not None}, custom_h={final_custom_h is not None}"
                    )
                    if page.views:
                        page.update()
                else:
                    ...
                    print(
                        f"[Bay Diagram] Failed to generate: diagram_b64={diagram_b64 is not None}, image={inputs.get('bay_diagram_image') is not None}, bw={bw}, bh={bh}, opening_w={opening_w}, opening_h={opening_h}"
                    )
                    if inputs.get("bay_diagram_container"):
                        inputs["bay_diagram_container"].visible = False
                    if page.views:
        ...
        door_only_diagram_index = [
            0
        ]  # which door (0-based) is shown; use list so refresh can update
            ...
            try:
                ...
                if not n:
                    ...
                    if b64:
                        ...
                        if inputs.get("door_only_title_text"):
                            inputs[
                                "door_only_title_text"
                            ].value = f"Door {idx + 1} of {n}"
        ...
        def door_only_diagram_next(e):
            ...
            if len(instances) <= 1:
                return
            door_only_diagram_index[0] = min(
                len(instances) - 1, door_only_diagram_index[0] + 1
            )
            refresh_door_only_diagram()
        ...
        def on_elevation_load(e):
            ...
            if not elev_name or elev_name == "New Elevation":
                ...
                inputs[
                    "saved_elev"
                ].value = "New Elevation"  # Keep "New Elevation" for consistency
            ...
            update_bay_visibility(
                None
            )  # This sets visibility and triggers update_dynamic_bay_inputs
        ...
        def render_doors():
            ...
            for i, door in enumerate(state["current_doors"]):
                ...
                if door.get("x_in") is not None:
                    ...
                    span_txt = (
                        "Length " + ", ".join(parts) + " occupied" if parts else None
                    )
                lines = [
                    ft.Text(f"Door {i + 1}", weight="bold", color=COLOR_TEXT),
                    ft.Text(
                        f"{door['size']} | {door['stile']} Stile | Qty: {door['count']}",
                        size=12,
                        color=COLOR_TEXT_DIM,
                    ),
                ]
                ...
                if hw_txt:
                    lines.append(
                        ft.Text(
                            f"HW: {hw_txt}", size=10, color=COLOR_TEXT_DIM, italic=True
                        )
                    )
                        ...
                        content=ft.Row(
                            [
                                ...
                                ft.IconButton(
                                    ft.Icons.EDIT,
                                    icon_color="blue",
                                    on_click=lambda e, idx=i: edit_door(idx),
                                ),
                                ft.IconButton(
                                    ft.Icons.DELETE,
                                    icon_color="red",
                                    on_click=lambda e, idx=i: delete_door(idx),
                                ),
                            ]
                        ),
                        bgcolor=COLOR_INPUT_BG,
        ...
        def save_doors_action():
            ...
            if elev_name:
                ...
                print(
                    "Warning: save_doors_action called but no elevation name in type input."
                )
        ...
        def _check_door_placement(opening_w, opening_h, doors_to_check):
            ...
            for door in doors_to_check:
                ...
                if dh > opening_h:
                    return (
                        False,
                        f'Door height ({dh / 12:.1f}") exceeds opening height ({opening_h / 12:.1f}").',
                    )
                for left, right, _dw, _dh in _spans_for_door(door, opening_w):
                    if left < 0:
                        return (
                            False,
                            f'Door extends past left edge (X position too far left). Use center position so door stays within 0-{opening_w:.1f}".',
                        )
                    if right > opening_w:
                        return (
                            False,
                            f'Door extends past right edge (X position too far right). Use center within 0-{opening_w:.1f}".',
                        )
            ...
            for i, (a1, a2) in enumerate(spans):
                for j, (b1, b2) in enumerate(spans):
                    ...
                    if not (a2 <= b1 or b2 <= a1):
                        return (
                            False,
                            "Doors overlap on the X axis. Change X positions or opening width.",
                        )
            return True, None

        def modify_door(action):
            # Auto-save elevation if updating a door on an existing elevation
            is_existing_elevation = (
                inputs["saved_elev"].value is not None
                and inputs["saved_elev"].value != ""
            )

            # Validate door count - must be a positive integer
            door_count_str = (
                inputs["door_count"].value.strip() if inputs["door_count"].value else ""
            )
            ...
            if count == 1:
                if not door_only_mode and len(x_values) < 1:
                    show_snack(
                        "X position (center, inches) is required so the door can be placed on the diagram."
                    )
                    return
                new_door = {
                ...
                if not door_only_mode and len(x_values) < count:
                    show_snack(
                        f"Enter {count} X positions (comma-separated) for center of each door."
                    )
                    return
                new_door = {
                    "size": inputs["door_size"].value,
            ...
            if door_only_mode:
                ...
                try:
                    opening_w = (
                        float(inputs["width"].value)
                        if inputs.get("width") and inputs["width"].value
                        else 0
                    )
                    opening_h = (
                        float(inputs["height"].value)
                        if inputs.get("height") and inputs["height"].value
                        else 0
                    )
                except (TypeError, ValueError):
                ...
                if opening_w <= 0 or opening_h <= 0:
                    show_snack(
                        "Enter Opening Width and Height first so door placement can be validated."
                    )
                    return
                if action == "add":
                    ...
                    if state["selected_door_index"] is not None and 0 <= state[
                        "selected_door_index"
                    ] < len(doors_to_check):
        ...
        def save_elevation_action(e):
            ...
            try:
                ...
                if not elev:
                    raise ValueError("Elevation Name Required")
                ...
                if door_only:
                    ...
                    total = 1
                    finish = (
                        state["finish_options"][0]
                        if state["finish_options"]
                        else "Clear"
                    )
                    system = "Other"
                ...
                show_qty_per_elev = (
                    inputs.get("qty_per_elev_checkbox", ft.Checkbox()).value
                    if inputs.get("qty_per_elev_checkbox")
                    else False
                )
                show_total_cost_per_elev = (
                    inputs.get("total_cost_per_elev_checkbox", ft.Checkbox()).value
                    if inputs.get("total_cost_per_elev_checkbox")
                    else False
                )
                show_discounted_cost_per_elev = (
                    inputs.get("discounted_cost_per_elev_checkbox", ft.Checkbox()).value
                    if inputs.get("discounted_cost_per_elev_checkbox")
                    else False
                )
                ...
                if door_only:
                    ...
                    def parse_csv(txt, total_len, count):
                        try:
                            ...
                            if len(parts) > count:
                                return []  # Fail silently or handle
                            # fill remainder
                    ...
                    if dynamic_w:
                        ...
                        if filled_count > 0:  # Custom mode active
                            ...
                            if blank_count > 0:
                                ...
                                if remaining < 0:
                                    raise ValueError(
                                        f"Custom widths sum ({filled_w_sum:.2f}) exceeds Opening Width ({w:.2f})"
                                    )
                                share = remaining / blank_count
                    ...
                    if dynamic_h:
                        ...
                        if filled_count > 0:
                            ...
                            if blank_count > 0:
                                ...
                                if remaining < 0:
                                    raise ValueError(
                                        f"Custom heights sum ({filled_h_sum:.2f}) exceeds Opening Height ({h:.2f})"
                                    )
                                share = remaining / blank_count
                                for f in dynamic_h:
                                    try:
                    ...
                    if w_vals and abs(sum(w_vals) - w) > 0.01:
                        raise ValueError(
                            f"Custom widths sum ({sum(w_vals):.2f}) does not match Opening Width ({w:.2f})"
                        )
                    if h_vals and abs(sum(h_vals) - h) > 0.01:
                        raise ValueError(
                            f"Custom heights sum ({sum(h_vals):.2f}) does not match Opening Height ({h:.2f})"
                        )
                    ...
                    proj_settings = (
                        load_project_settings(state["current_project"]) or {}
                    )
                ...
                if state["current_doors"]:
                    door_items = calculate_door_info(
                        state["current_doors"], finish=data["finish"], total_count=total
                    )
                    data["calculated_outputs"].extend(door_items)
                # Persist raw doors for Excel diagrams (bay diagram green boxes, door-only diagrams)
                data["doors"] = (
                    list(state["current_doors"]) if state.get("current_doors") else []
                )
                ...
                try:
                    db.save_elevations(
                        state["current_project"], state["saved_elevations"]
                    )
                ...
                try:
                    ...
                    paths = prepare_temp_files_for_excel(
                        state["current_project"],
                        elevations_override=state["saved_elevations"],
                    )
                ...
                dropdown_options = [ft.dropdown.Option("New Elevation")] + [
                    ft.dropdown.Option(x) for x in new_opts
                ]
                ...
                if is_update:
                    ...
                    inputs[
                        "saved_elev"
                    ].value = "New Elevation"  # Set to "New Elevation" instead of None
        ...
        def duplicate_elevation_action(e):
            ...
            if (
                not elev_name
                or elev_name == "New Elevation"
                or elev_name not in state["saved_elevations"]
            ):
            ...
            dlg = ft.AlertDialog(
                ...
                content=ft.Container(content=new_name_field, width=400, padding=10),
                actions=[
            ...
            try:
                ...
                if not excel_path or not os.path.exists(excel_path):
                    ...
                    excel_path = os.path.join(
                        "reports", f"{state['current_project']}_{ts}.xlsx"
                    )
                    os.makedirs("reports", exist_ok=True)
                ...
                pdf_path = export_project_to_pdf(
                    state["current_project"], excel_path=excel_path, include_logo=True
                )
                ...
                if state["saved_elevations"]:
                    try:
                        ...
                        temp_report_path = os.path.join(
                            "reports", f"{state['current_project']}_temp_{ts}.xlsx"
                        )
                        ...
                        if has_percentages:
                            show_snack(
                                "Additional cost settings saved and report updated. Please close and reopen Excel to see changes.",
                                "green",
                            )
                        else:
                            show_snack(
                                "Settings saved (all percentages are 0%)", "orange"
                            )
                    except Exception as report_err:
        ...
        def save_markup_settings(e):
            ...
            try:
                settings = {
                    k: get_input_pct(k)
                    for k in [
                        "profit_on_material_pct",
                        "profit_on_waste_pct",
                        "profit_on_glass_pct",
                        "profit_on_wages_pct",
                        "planning_technical_pct",
                        "commission_pct",
                    ]
                }
                ...
                if state["saved_elevations"]:
                    try:
                        ...
                        if has_percentages:
                            show_snack(
                                "Markup settings saved and report updated. Please close and reopen Excel to see changes.",
                                "green",
                            )
                        else:
                            show_snack(
        ...
        def save_elevation_summary_settings(e):
            ...
            try:
                settings = {
                    "show_elevation_names": inputs.get(
                        "show_elevation_names", ft.Checkbox()
                    ).value
                    if inputs.get("show_elevation_names")
                    else False,
                    "show_elevation_quantity": inputs.get(
                        "show_elevation_quantity", ft.Checkbox()
                    ).value
                    if inputs.get("show_elevation_quantity")
                    else False,
                    "show_elevation_dimensions": inputs.get(
                        "show_elevation_dimensions", ft.Checkbox()
                    ).value
                    if inputs.get("show_elevation_dimensions")
                    else False,
                    "show_elevation_sqft": inputs.get(
                        "show_elevation_sqft", ft.Checkbox()
                    ).value
                    if inputs.get("show_elevation_sqft")
                    else False,
                    "show_elevation_perimeter": inputs.get(
                        "show_elevation_perimeter", ft.Checkbox()
                    ).value
                    if inputs.get("show_elevation_perimeter")
                    else False,
                }
                    ...
                    try:
                        ...
                        if os.path.exists(temp_report_path):
                            ...
                            try:
                                ...
                                pass
                        show_snack(
                            "Elevation summary settings saved and report updated",
                            "green",
                        )
                    except Exception as report_err:
                        show_snack("Settings saved, but report update failed", "orange")
        ...
        misc_section = ft.Column(
            [
                ...
                ft.Text(
                    "Enter percentages for summary calculations (leave blank for 0%)",
                    size=12,
                    color=COLOR_TEXT_DIM,
                    italic=True,
                ),
                    ...
                    [
                        ...
                        ft.Column(
                            [
                                ...
                                ft.Container(
                                    height=48
                                ),  # Spacer to align with 4th field
                            ],
                            spacing=15,
                            expand=True,
                        ),
        ...
        markup_section = ft.Column(
            [
                ...
                ft.Text(
                    "Enter percentages for markup calculations (leave blank for 0%)",
                    size=12,
                    color=COLOR_TEXT_DIM,
                    italic=True,
                ),
        ...
        elevation_summary_section = ft.Column(
            [
                ...
                ft.Text(
                    "Select which columns to display in the summary report",
                    size=12,
                    color=COLOR_TEXT_DIM,
                    italic=True,
                ),
        ...
        inputs["show_elevation_names"].value = project_settings.get(
            "show_elevation_names", False
        )
        inputs["show_elevation_quantity"].value = project_settings.get(
            "show_elevation_quantity", False
        )
        inputs["show_elevation_dimensions"].value = project_settings.get(
            "show_elevation_dimensions", False
        )
        inputs["show_elevation_sqft"].value = project_settings.get(
            "show_elevation_sqft", False
        )
        inputs["show_elevation_perimeter"].value = project_settings.get(
        ...
        def refresh_waste_calculator():
            ...
            if not state.get("current_project"):
                ...
                # Debug output
                print(
                    f"[Waste Calculator Frontend] Elevations: {len(elevations_data)} items"
                )
                print(
                    f"[Waste Calculator Frontend] Extra materials: {len(extra_materials_data)} items"
                )
                if extra_materials_data:
                    print(
                        f"[Waste Calculator Frontend] Extra materials keys: {list(extra_materials_data.keys())[:5]}"
                    )
                ...
                try:
                    ...
                    print(
                        f"[Waste Calculator Frontend] Material breakdown: {len(waste_stats.get('material_breakdown', []))} items"
                    )
                except Exception as e:
            ...
            total_material_cost_text.value = (
                f"${waste_stats['total_material_cost']:.2f}"
            )
                ...
                breakdown_rows.append(
                    ft.DataRow(
                        cells=[
                            ...
                            ft.DataCell(
                                ft.Text(
                                    f"{material['waste_percentage']:.1f}%",
                                    size=11,
                                    color=get_waste_percentage_color(
                                        material["waste_percentage"]
                                    ),
                                )
                            ),
                            ft.DataCell(
                                ft.Text(f"${material['waste_cost']:.2f}", size=11)
                            ),
                            ft.DataCell(
                                ft.Text(
                                    material.get(
                                        "waste_quantity_display",
                                        f"{material['waste_quantity']:.2f} {material['unit']}",
                                    ),
                                    size=11,
                                )
                            ),
        ...
        waste_percentage_text = ft.Text(
            "0.00%", size=24, weight="bold", color=COLOR_ACCENT
        )
        waste_progress = ft.ProgressBar(
            value=0.0, width=400, height=20, color=COLOR_ACCENT
        )
        ...
        waste_breakdown_table = ft.DataTable(
            columns=[
                ft.DataColumn(
                    ft.Text("Material", size=11, weight="bold", color=COLOR_ACCENT)
                ),
                ft.DataColumn(
                    ft.Text("Waste %", size=11, weight="bold", color=COLOR_ACCENT)
                ),
                ft.DataColumn(
                    ft.Text("Waste Cost", size=11, weight="bold", color=COLOR_ACCENT)
                ),
                ft.DataColumn(
                    ft.Text("Waste Qty", size=11, weight="bold", color=COLOR_ACCENT)
                ),
            ],
            rows=[],
        ...
        waste_calculator_section = ft.Column(
            [
                ...
                ft.Text(
                    "Visual waste analysis and optimization suggestions",
                    size=12,
                    color=COLOR_TEXT_DIM,
                    italic=True,
                ),
                    ...
                    content=ft.Column(
                        [
                            ft.Row(
                                [
                                    ft.Icon(
                                        ft.Icons.ANALYTICS, color=COLOR_ACCENT, size=20
                                    ),
                                    ft.Text(
                            ...
                            ft.Row(
                                [
                                    ...
                                    ft.Container(content=waste_progress, expand=True),
                                ],
                                alignment=ft.MainAxisAlignment.START,
                            ),
                            ft.Container(height=20),
                    ...
                    content=ft.Column(
                        [
                            ft.Row(
                                [
                                    ft.Icon(
                                        ft.Icons.LIGHTBULB, color=COLOR_ACCENT, size=20
                                    ),
                                    ft.Text(
                                        "Optimization Suggestions",
                                        size=14,
        ...
        _def = lambda k, d: (
            str(project_settings.get(k, d))
            if project_settings.get(k) not in (None, "")
            else str(d)
        )
        create_input_field(
            "Discount multiplier (projects < $50k)",
            "discount_multiplier_low",
            numeric=True,
            expand=False,
            value=_def("discount_multiplier_low", 0.614),
        )
        create_input_field(
            "Discount multiplier (projects >= $50k)",
            "discount_multiplier_high",
            numeric=True,
            expand=False,
            value=_def("discount_multiplier_high", 0.572),
        )
        create_input_field(
            "Discount threshold ($)",
            "discount_threshold",
            numeric=True,
            expand=False,
            value=_def("discount_threshold", 50000),
        )
        create_input_field(
            "Glass per sqft ($)",
            "glass_per_sqft",
            numeric=True,
            expand=False,
            value=_def("glass_per_sqft", 10.5),
        )
        create_input_field(
            "Fabrication cost per joint ($)",
            "fabrication_cost_per_joint",
            numeric=True,
            expand=False,
            value=_def("fabrication_cost_per_joint", 15),
        )
        ...
        pricing_adjustment_container = ft.Container(
            content=ft.Column(
                [
                    ft.Row(
                        [
                            ...
                            ft.ElevatedButton(
                                ...
                                bgcolor=COLOR_ACCENT,
                                color="white",
                                on_click=save_pricing_adjustment,
                                height=40,
                                width=120,
                            ),
                        ],
                    ...
                    ft.Text(
                        "Adjust discount multipliers, glass per sqft, and fabrication cost. Values shown are defaults until changed.",
                        size=12,
                        color=COLOR_TEXT_DIM,
                        italic=True,
                    ),
                ...
                ft.Row(
                    [
                        ft.ElevatedButton(
                            ...
                            color="white",
                            on_click=lambda e: (
                                state.__setitem__("report_intent", "pdf"),
                                page.go("/report-options"),
                            ),
                        ...
                        ft.ElevatedButton(
                            ...
                            color="white",
                            on_click=lambda e: (
                                state.__setitem__("report_intent", "excel"),
                                page.go("/report-options"),
                            ),
                        ),
                    ],
        ...
        saved_dd = create_dropdown(
            "Load Elevation", "saved_elev", dd_options, on_change=on_elevation_load
        )
        ...
        hardware_cbs = {
            opt: ft.Checkbox(label=opt, fill_color=COLOR_ACCENT)
            for opt in state["hardware_options"]
        }
        ...
        door_manager_content = ft.Column(
            [
                ft.Row(
                    [
                        create_dropdown("Size", "door_size", state["door_options"]),
                        create_input_field("Count (Per Elevation)", "door_count"),
                    ]
                ),
                assign_ref(
                    "door_x_pos_container",
                    ft.Container(
                        content=create_input_field(
                            'X position (center, ") — one value or comma-separated for multiple',
                            "door_x_pos",
                        ),
                        visible=True,
                    ),
                ...
                ft.Row(
                    [
                        ft.ElevatedButton(
                            ...
                            bgcolor="#4CAF50",
                            color="white",
                            on_click=lambda e: modify_door("add"),
                            expand=True,
                        ),
                        ft.ElevatedButton(
                            "UPDATE DOOR",
                            bgcolor="#FF9800",
                            color="white",
                            on_click=lambda e: modify_door("update"),
                            expand=True,
                        ),
                    ]
                ),
        ...
        def toggle_door_manager(e):
            ...
            door_manager_icon.icon = (
                ft.Icons.EXPAND_MORE
                if door_manager_expanded
        ...
        form_col = ft.Column(
            [
                ft.Text(
                    "ELEVATION DETAILS", size=14, weight="bold", color=COLOR_ACCENT
                ),
                ft.Row(
                    [inputs["toggle_elevation_btn"], inputs["toggle_door_only_btn"]],
                    spacing=0,
                ),
                ft.Row([create_input_field("Elevation Type (Name)", "type")]),
                # Rest of form - hidden when door_only
                assign_ref(
                    "door_only_rest_container",
                    ft.Column(
                        [
                            ft.Row(
                                [
                                    create_dropdown(
                                        "System",
                                        "system",
                                        state["system_options"],
                                        on_change=update_bay_visibility,
                                    ),
                                    create_dropdown(
                                        "Finish", "finish", state["finish_options"]
                                    ),
                                ]
                            ),
                            ft.Row(
                            ...
                            assign_ref(
                                "custom_w_container",
                                ft.Container(
                                    content=inputs.setdefault(
                                        "custom_w_col", ft.Column([], spacing=10)
                                    ),
                                    visible=False,
                                ),
                            ),
                            assign_ref(
                                ...
                                ft.Container(
                                    content=inputs.setdefault(
                                        "custom_h_col", ft.Column([], spacing=10)
                                    ),
                                    visible=False,
                                ),
                            ),
                        ...
                        [
                            assign_ref(
                                ...
                                ft.ElevatedButton(
                                    ...
                                    bgcolor=COLOR_ACCENT,
                                    color="white",
                                    on_click=save_elevation_action,
                                    expand=True,
                                    height=50,
                                ),
                            ),
                            assign_ref(
                                ...
                                ft.IconButton(
                                    ...
                                    icon_color=COLOR_ACCENT,
                                    tooltip="Duplicate Elevation",
                                    on_click=duplicate_elevation_action,
                                    visible=False,
                                ),
                            ),
                            ft.IconButton(
                                ...
                                icon_color="red",
                                tooltip="Delete Elevation",
                                on_click=delete_elevation_action,
                            ),
                        ]
                    ),
        ...
        bay_diagram_image = inputs.setdefault(
            "bay_diagram_image",
            ft.Image(src_base64="", fit=ft.ImageFit.CONTAIN, visible=False),
        )
        ...
        door_only_diagram_image = ft.Image(
            src_base64="", fit=ft.ImageFit.CONTAIN, visible=False
        )
        ...
        door_only_prev_btn = ft.IconButton(
            ft.Icons.CHEVRON_LEFT,
            icon_color=COLOR_ACCENT,
            on_click=door_only_diagram_prev,
            tooltip="Previous door",
        )
        door_only_next_btn = ft.IconButton(
            ft.Icons.CHEVRON_RIGHT,
            icon_color=COLOR_ACCENT,
            on_click=door_only_diagram_next,
            tooltip="Next door",
        )
        assign_ref(
            "door_only_nav_row",
        ...
        def on_tab_change(e):
            ...
            if e.control.selected_index == 1:  # Summary tab is index 1
                try:
                    ...
                    print(
                        f"[WARNING] Error refreshing waste calculator on tab change: {ex}"
                    )
        ...
        tabs = ft.Tabs(
            ...
            tabs=[
                    ...
                    content=ft.Row(
                        [
                            ft.Container(
                                content=ft.Column(
                                    [
                                        ft.Container(content=saved_dd, padding=0),
                                        form_col,
                                    ],
                                    spacing=5,
                                ),
                ...
                ft.Tab(
                    text="Summary",
                    content=ft.Container(
                        content=summary_settings_container, padding=20, expand=True
                    ),
                ),
                ft.Tab(text="Pricing Adjustment", content=pricing_adjustment_container),
            ],
            expand=True,
        )
        ...
        if inputs.get("custom_w_container"):
            inputs["custom_w_container"].visible = show_bays_init
        if inputs.get("custom_h_container"):
            inputs["custom_h_container"].visible = show_bays_init
        ...
        if is_yes45_init:
            ...
            def populate_dynamic_fields_init(count, prefix, col_key, store_key):
                ...
                if count > 0:
                    inputs[col_key].controls.append(
                        ft.Text(
                            f"Custom Bay {'Widths' if 'w' in prefix else 'Heights'} (leave blank to auto-fill)",
                            size=12,
                            color=COLOR_TEXT_DIM,
                        )
                    )
                    ...
                    for i in range(count):
                        ...
                        field = create_input_field(
                            f"Bay {i + 1} {'Width' if 'w' in prefix else 'Height'}",
                            f"{prefix}_{i}",
                            expand=True,
                            value=val,
                        )
    ...
    def build_report_options_view():
        """Multi-layer stock list for report customization.
        Layer 1: Which sheets (elevations + summary)
        Layer 2: Per-elevation sections — each is a dropdown
        Layer 3: When a section is checked, its relevant columns appear
        """
        elev_names = sorted(state.get("saved_elevations", {}).keys())
        elev_count = len(elev_names)

        # ── Section definitions ──
        # Material sections that can have sub-columns
        MATERIAL_SECTIONS = [
            "profiles",
            "accessories",
            "gaskets",
        ...
        # Non-material sections (no sub-columns)
        OTHER_SECTIONS = ["system_input", "elevation_cost_summary", "diagram"]
        ALL_SECTIONS = MATERIAL_SECTIONS + OTHER_SECTIONS
        ...
        # ── Summary column definitions per category ──
        # Each summary material section has its own full column list.
        ...
        # Full per-category summary column definitions
        SUMMARY_CATEGORY_COLS = {
            "profiles": _SUM_COMMON
            + [
        ...
        # ── State containers ──
        elev_included_cbs = {}  # {elev_name: Checkbox}
        elev_section_cbs = {}  # {elev_name: {section_key: Checkbox}}
        elev_col_cbs = {}  # {elev_name: {section_key: {col_key: Checkbox}}}
        # For sections with sub-columns, containers that show/hide
        elev_col_containers = {}  # {elev_name: {section_key: Container}}
        summary_included_cb = None
        summary_section_cbs = {}  # {section_key: Checkbox}
        summary_col_cbs = {}  # {section_key: {col_key: Checkbox}}
        summary_col_containers = {}  # {section_key: Container}
        summary_cost_cbs = {}  # {cost_key: Checkbox}

        def _make_section_with_columns(
            section_key, sec_cb, col_cbs_dict, col_container_ref, col_defs
        ):
            """Build a section checkbox with a collapsible column area underneath.
            col_cbs_dict is now {section_key: {col_key: Checkbox}} for per-section storage."""
            # Create column checkboxes for this specific section
            section_col_cbs = {}
            col_controls = []
            for col_key, col_label in col_defs:
                cb = ft.Checkbox(label=col_label, value=True, fill_color=COLOR_ACCENT)
                section_col_cbs[col_key] = cb
                col_controls.append(cb)
            # Store under the section key instead of flat merge
            col_cbs_dict[section_key] = section_col_cbs

            col_column = ft.Column(col_controls, spacing=2)
            col_container = ft.Container(
                content=ft.Container(
                    content=col_column,
                    padding=ft.padding.only(left=28, top=4, bottom=4),
                ),
                visible=sec_cb.value,
            )
            col_container_ref[section_key] = col_container

            def on_section_change(e, _cont=col_container):
                _cont.visible = sec_cb.value
                page.update()

            sec_cb.on_change = on_section_change

            return ft.Column([sec_cb, col_container], spacing=0)
        ...
        def make_elev_panel(elev_name):
            ...
            # Get this elevation's own count for per-elevation column visibility
            elev_data = state.get("saved_elevations", {}).get(elev_name, {})
            this_total_count = elev_data.get("total_count", 1)
            try:
                this_total_count = int(this_total_count)
            except (TypeError, ValueError):
                this_total_count = 1

            sec_cbs = {}
            col_cbs = {}  # {section_key: {col_key: Checkbox}}
            col_containers = {}
            section_controls = []
            ...
            for sk in ALL_SECTIONS:
                sec_cb = ft.Checkbox(
                    ...
                    value=SECTION_DEFAULTS[sk],
                    fill_color=COLOR_ACCENT,
                )
                sec_cbs[sk] = sec_cb

                if sk in MATERIAL_SECTIONS:
                    # Material section: per-section columns with this elevation's count
                    elev_cols = _build_elev_cols(sk, this_total_count)
                    widget = _make_section_with_columns(
                        sk, sec_cb, col_cbs, col_containers, elev_cols
                    )
                    section_controls.append(widget)
                else:
                    # Non-material: just the checkbox
                    section_controls.append(sec_cb)

            elev_section_cbs[elev_name] = sec_cbs
            elev_col_cbs[elev_name] = col_cbs
            elev_col_containers[elev_name] = col_containers

            qty_note = (
                ft.Text(
                    "  'Per Elevation' columns only apply when count > 1",
                    color=COLOR_TEXT_DIM,
                    size=10,
                    italic=True,
                )
                if this_total_count > 1
                else ft.Container()
            )
            ...
            expanded_content = ft.Column(
                [
                    ft.Text(
                        "Sections & Columns", size=12, weight="bold", color=COLOR_ACCENT
                    ),
                    ft.Column(section_controls, spacing=2),
                    qty_note,
                ],
                spacing=4,
            )
            ...
            return ft.ExpansionPanel(
                header=ft.Row(
                    [inc_cb, ft.Text(elev_name, color=COLOR_TEXT, expand=True)],
                    spacing=10,
                    alignment=ft.MainAxisAlignment.START,
                ),
        ...
        def make_summary_panel():
            nonlocal summary_included_cb, summary_section_cbs, summary_col_cbs
            nonlocal summary_col_containers, summary_cost_cbs
            ...
            section_controls = []
            for sk in MATERIAL_SECTIONS:
                sec_cb = ft.Checkbox(
                    label=SECTION_LABELS[sk],
                    value=True,
                    fill_color=COLOR_ACCENT,
                )
                summary_section_cbs[sk] = sec_cb

                # Build category-specific columns from full definition
                cat_cols = SUMMARY_CATEGORY_COLS.get(sk, [])
                cat_col_cbs = {}
                widget = _make_section_with_columns(
                    sk, sec_cb, cat_col_cbs, summary_col_containers, cat_cols
                )
                # _make_section_with_columns stores as {sk: {col_key: Checkbox}},
                # extract the inner dict for summary_col_cbs
                summary_col_cbs[sk] = cat_col_cbs.get(sk, {})
                section_controls.append(widget)

            # Cost overview
            cost_controls = []
            for ck in COST_OVERVIEW_KEYS:
                cb = ft.Checkbox(
                    label=COST_OVERVIEW_LABELS[ck],
                    value=True,
                    fill_color=COLOR_ACCENT,
                )
                summary_cost_cbs[ck] = cb
                cost_controls.append(cb)

            return ft.ExpansionPanel(
                header=ft.Row(
                    [
                        summary_included_cb,
                        ft.Text("Summary", color=COLOR_TEXT, expand=True),
                    ],
                    spacing=10,
                    alignment=ft.MainAxisAlignment.START,
                ),
                content=ft.Container(
                    content=ft.Column(
                        [
                            ...
                            ft.Column(cost_controls, spacing=4),
                        ],
                        spacing=8,
                    ),
                    padding=12,
        ...
        ep_list = ft.ExpansionPanelList(
            controls=all_panels, spacing=4, expand_icon_color=COLOR_ACCENT
        )
        ...
        def apply_to_all(e):
            if not elev_names:
                return
            src = elev_names[0]
            if src not in elev_section_cbs or src not in elev_col_cbs:
                return
            for name in elev_names[1:]:
                if name in elev_section_cbs:
                    for k in ALL_SECTIONS:
                        elev_section_cbs[name][k].value = elev_section_cbs[src][k].value
                        # Update column container visibility
                        if k in MATERIAL_SECTIONS:
                            if (
                                name in elev_col_containers
                                and k in elev_col_containers[name]
                            ):
                                elev_col_containers[name][k].visible = elev_section_cbs[
                                    src
                                ][k].value
                if name in elev_col_cbs:
                    # Copy per-section column checkbox states
                    for sk in MATERIAL_SECTIONS:
                        src_sec_cbs = elev_col_cbs[src].get(sk, {})
                        dst_sec_cbs = elev_col_cbs[name].get(sk, {})
                        for col_key, cb in src_sec_cbs.items():
                            if col_key in dst_sec_cbs:
                                dst_sec_cbs[col_key].value = cb.value
            ...
            try:
                ...
                for name in elev_names:
                    ...
                    per_elev_sections[name] = {
                        k: elev_section_cbs[name][k].value for k in ALL_SECTIONS
                    }
                    # Collect per-section column states
                    # elev_col_cbs[name] is {section_key: {col_key: Checkbox}}
                    per_section_cols = {}
                    for sk in MATERIAL_SECTIONS:
                        sec_col_cbs = elev_col_cbs[name].get(sk, {})
                        per_section_cols[sk] = {
                            col_key: cb.value for col_key, cb in sec_col_cbs.items()
                        }
                    per_elev_columns[name] = per_section_cols

                # Read summary options
                summary_options = {}
                if summary_included:
                    # Summary sections
                    sum_sec = {}
                    for sk in MATERIAL_SECTIONS:
                        if sk in summary_section_cbs:
                            sum_sec[sk] = summary_section_cbs[sk].value
                    summary_options["summary_sections"] = sum_sec

                    # Summary columns — flatten all category column states
                    sum_cols = {}
                    for sk in MATERIAL_SECTIONS:
                        cat_cbs = summary_col_cbs.get(sk, {})
                        for col_key, cb in cat_cbs.items():
                            # OR across categories: if any category has it on, it's on
                            sum_cols[col_key] = sum_cols.get(col_key, False) or cb.value
                    summary_options["summary_columns"] = sum_cols
                ...
                # Build include_sections for Excel generator
                _KEY_MAP = {"fabrication": "fabrication", "diagram": "diagrams"}
                ...
                # Attach report_options and project_name to report_data
            ...
            [
                ft.Row(
                    [
                        ft.IconButton(
                            ft.Icons.ARROW_BACK,
                            on_click=lambda e: page.go("/workspace"),
                            icon_color=COLOR_TEXT,
                        ),
                        ft.Text(
                            "Report Stock List",
                ...
                ft.Text(
                    "Select elevations and Summary to include. Expand to configure sections — check a section to reveal its columns.",
                    color=COLOR_TEXT_DIM,
                    size=12,
                ),
                ft.Container(height=8),
