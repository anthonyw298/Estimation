...
def generate_excel_report(
    ...
    calculated_outputs, doors, completion_callback=None, reset=False, delete_elevation_type=None,
    mode=None
):
    ...
    if delete_elevation_type:
        ...
        try:
            ...
            if completion_callback:
                completion_callback(f"Error saving updated elevations after delete: {e}")
            return
        ...
        current_saved_elevations[elevation_type] = {
            ...
            "calculated_outputs": calculated_outputs, "material_impact": [], "doors": doors
        }
    ...
    if not sorted_elev_names:
        ...
        for elev_name in sorted_elev_names:
            ...
            if elev_data.get("doors") is not None:
                door_info = []
                for door in elev_data["doors"]:
                    hardware_str = ", ".join(door['hardware'])
                    door_info.append(f"{door['count']}x {door['size']} {door['stile']} with {hardware_str}")
                input_data.append(("Doors", "\n".join(door_info)))
            ...
            # This is the corrected section to write the SYSTEM TOTAL
    ...
    create_summary_sheet(excel_path, elevations_json_path, extra_materials_json_path)

    _autofit_columns(ws, 1, 8, end_row=ws.max_row + 2)
    _clean_trailing_blank_rows(ws, 1)

    try:
        wb.save(excel_path)
        if completion_callback:
            completion_callback(f"Excel report successfully generated at {excel_path}.")
    except Exception as e:
        if completion_callback:
            completion_callback(f"Error saving Excel file: {e}")