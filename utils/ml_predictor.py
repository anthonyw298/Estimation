...
class SimplePredictor:
    ...
    def add_sample(
        self, elevation_data: Dict, project_name: str = "", elevation_name: str = ""
    ) -> bool:
    ...
    def predict(self, elevation_data: Dict) -> Dict:
        ...
        for sample in self.training_data:
            if (
                abs(sample.get("width", 0) - width) < 1
                and abs(sample.get("height", 0) - height) < 1
                and sample.get("bays", 0) == bays
            ):
                return {
        ...
        if self.training_data:
            ...
            avg_cost_per_sqft = sum(
                s["cost"] / max(1, s["sqft"]) for s in self.training_data
    ...
    def get_statistics(self) -> Dict:
        ...
        avg_cost_per_sqft = round(total_cost / total_sqft, 2) if total_sqft > 0 else 0
            ...
            "common_configurations": [
                {
                    "config": c[0],
                    "count": c[1],
                    "percentage": round(c[1] / len(self.training_data) * 100, 1),
                }
    ...
    def remove_samples_by_project(self, project_name: str) -> int:
        ...
        self.training_data = [
            s for s in self.training_data if s.get("project") != project_name
        ]
        ...
        if removed_count > 0:
            ...
            self._save_data()
            print(
                f"[ML] Removed {removed_count} training samples for project '{project_name}'"
            )
...
# Public API
def add_project_to_training(
    elevation_data: Dict, project_name: str = "", elevation_name: str = ""
) -> bool:
...
def remove_elevation_from_training(
    elevation_data: Dict, project_name: str = "", elevation_name: str = ""
) -> bool:
    ...
    sample_id = (
        f"{project_name}_{elevation_name}_{width}_{height}_{bays_w}x{bays_h}_{finish}"
    )

    initial_count = len(predictor.training_data)
    predictor.training_data = [
        s for s in predictor.training_data if s.get("id") != sample_id
    ]
